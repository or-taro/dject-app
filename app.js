...
let novel=null,chapter=null,chapters=[],job=null,dirtyN=0,dirtyC=0,queue=null,timer=null,blocked=false;
...
async function openNovel(id,selected){
 ...
 await list();await checkJob(false);
}
function showJob(result){
 job=result?.job||null;const labels={pending:'GPT 작성 대기',completed:'결과 도착',conflict:'결과 검토 필요',superseded:'새 작업으로 교체됨'};
...
async function checkJob(refresh){
 if(!novel)return;const result=await api('jobs.current');showJob(result);
 if(refresh&&!dirtyN&&!dirtyC&&!queue&&!blocked&&result?.job?.chapter_id===chapter.id&&result.job.status==='completed'){
  const expectedId=chapter.id;const c=await api('chapters.get',{id:expectedId});if(chapter?.id===expectedId&&!dirtyN&&!dirtyC&&!queue&&!blocked&&c.version!==chapter.version)renderChapter(c);
 }
}
let running=false;
...
$('publishJob').onclick=()=>run(async()=>{await flush();const published=await api('jobs.create',{id:chapter.id});showJob({job:published});say('GPT용 작업을 저장했습니다. 개인 GPT에서 요청문을 보내 주세요.');});
...
$('checkJob').onclick=()=>run(async()=>{await checkJob(true);say('작업 상태를 확인했습니다.');});
